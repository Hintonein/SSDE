"""Parallelized, single-point launch script to run ssde on a set of benchmarks."""
...
def train_ssde(config):
    """Trains ssde and returns dict of reward, expression, and traversal"""
    ...
    if config["task"]["task_type"] == "control" and config["training"]["n_cores_batch"] > 1:
        import gym
        import ssde.task.control # Registers custom and third-party environments
...
def main(config_template, runs, n_cores_task, seed, benchmark, exp_name):
    """Runs ssde in parallel across multiple seeds using multiprocessing."""
    ...
    if n_cores_task > 1:
        ...
        for i, (result, summary_path) in enumerate(pool.imap_unordered(train_ssde, configs)):
        ...
        for i, config in enumerate(configs):
            result, summary_path = train_ssde(config)
            if not safe_update_summary(summary_path, result):
                print("Warning: Could not update summary stats at {}".format(summary_path))
            print("INFO: Completed run {} of {} in {:.0f} s".format(i + 1, runs, result["t"]))
