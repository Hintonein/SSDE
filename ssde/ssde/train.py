...
from ssde.utils import empirical_entropy, get_duration, weighted_quantile, pad_action_obs_priors
from ssde.memory import Batch, make_queue
from ssde.variance import quantile_variance
import pickle
# import psutil
...
def work(p):
    ...
    try:
        r = p.r
        if not check_serializable(p):
            return {"error":404, "program": None}
        return {"error": None, "program": p}
    except Exception as e:
        # 捕获异常并返回错误信息
        return {"error": str(e), "program": p}
...
class Trainer():
    def __init__(self, sess, policy, policy_optimizer, gp_controller, logger,
        ...
        if hasattr(self.policy_optimizer, 'pqt_k'):
            from ssde.policy_optimizer.pqt_policy_optimizer import PQTPolicyOptimizer
    ...
    def run_one_step(self, override=None):
        ...
        if self.pool is not None:
            ...
            for result in self.pool.map(work, programs_to_optimize):
                if result["error"] == 404:
                    raise BrokenPipeError
                else:
                    if result["error"] is not None:
                        print("One error in sub process:", result["error"])
                    pool_p_dict[result["program"].str] =  result["program"]
            programs = [pool_p_dict[p.str] if "r" not in p.__dict__  else p for p in programs]
        ...
        if self.epsilon is not None and self.epsilon < 1.0:
            ...
            if Program.task.task_type != 'regression' and Program.task.X_u_train is not None and not Program.task.ablation:
                if Program.task.task_type =='solver':
                    for p in programs:
                        p.optimize(reoptim=True)
                    r = np.array([p.r for p in programs])
                    r_max = np.max(r)
                    quantile = np.min(r)
                elif Program.task.task_type == 'recursion':
                    for p in programs:
                        p.recursion_optimize(reoptim=True)
                    r = np.array([p.r for p in programs])
                    r_max = np.max(r)
                    quantile = np.min(r)
