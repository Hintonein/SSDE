...
from ssde.library import Polynomial, StateChecker
...
class ssdeLeastSquaresRegressor(PolyRegressorMixin):
    """
    ...
    def fit(self, X, y, X_signature=None):
        ...
        if X_signature not in self.data_dict.keys():
            ...
            self.data_dict[X_signature] = ssdeLeastSquaresData(X, self.cutoff_p_value_ < 1.0 or
...
class ssdeLassoRegressor(PolyRegressorMixin):
    """
    ...
    def fit(self, X, y, X_signature=None):
        ...
        if X_signature not in self.data_dict.keys():
            while len(self.data_dict) >= self.n_max_records:
                self.delete_oldest_pair(self.data_dict)
            self.data_dict[X_signature] = ssdeLassoRegressorData(X)
        # perform lasso fit
...
regressors = {
        ...
        "lasso": Lasso,
        "ridge": Ridge,
        "ssde_least_squares" : ssdeLeastSquaresRegressor,
        "ssde_lasso" : ssdeLassoRegressor,
    }
...
class PolyOptimizer(PolyRegressorMixin):
    ...
    def fit(self, X, y):
        ...
        try:
            # perform fit; pass monomial data signature if using custom ssde optimizers
            if isinstance(self.regressor, (ssdeLeastSquaresRegressor,)):
    ...
    def clear(self):
        ...
        """
        self.data_dict.clear()
        if isinstance(self.regressor, (ssdeLeastSquaresRegressor, ssdeLassoRegressor)):
