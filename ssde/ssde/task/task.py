...
from ssde.utils import import_custom_source
from ssde.subroutines import parents_siblings
...
class HierarchicalTask(Task):
    ...
    def get_next_obs(self, actions, obs, already_finished):
        ...
        just_finished = (dangling == 0
                         )  # Trees that completed _this_ time step
        # [batch_size]
        ...
        next_obs = np.stack([action, parent, sibling, dangling],
                            axis=1)  # (?, 4)
    ...
    def reset_task(self, prior):
        ...
        initial_obs = np.array([
            self.library.EMPTY_ACTION, self.library.EMPTY_PARENT,
...
def make_task(task_type, **config_task):
    ...
    if task_type == "regression":
        from ssde.task.regression.regression import RegressionTask
        task_class = RegressionTask
    elif task_type == "solver":
        from ssde.task.solver.solver import SolverTask
        task_class = SolverTask
    elif task_type == "recursion":
        from ssde.task.recursion.recursion import RecursionTask
        task_class = RecursionTask
    else:
        # Custom task import
        task_class = import_custom_source(task_type)
        assert issubclass(task_class, Task), \
            "Custom task {} must subclass ssde.task.Task.".format(task_class)
...
def set_task(config_task):
    ...
    protected = config_task[
        "protected"] if "protected" in config_task else False
