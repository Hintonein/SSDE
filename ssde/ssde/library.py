...
class Token():
    ...
    def __init__(self, function, name, arity, complexity, input_var=None, torch_func=None):
    ...
    def __call__(self, *args, torch_flag=False):
...
class PlaceholderConstant(Token):
    ...
    def __init__(self, value=None):
        torch_value = None
        # vec_flag is 1 if value is a scalar, otherwise it is the length of the vector
        self.vec_flag = 0
        if value is not None:
            ...
            if isinstance(value,Tensor):
                torch_value = value
                value = np.atleast_1d(value.detach().numpy())
            else:
        ...
        super().__init__(function=self.function, name="const", arity=0, complexity=1, torch_func=self.torch_func)
    ...
    def torch_func(self):
        assert self.torch_value is not None, \
            "Constant is not callable with value None."
        if isinstance(self.torch_value, Tensor):
            return self.torch_value
        else:
            return tensor(self.torch_value)
    
    def const_prune(self, threshold=1e-3):
        # prune the constant value from vector to a single value
        assert self.vec_flag == 1, "Constants don't need to be pruned."
        if np.var(self.value, axis=1) < threshold:
            self.value = np.mean(self.value, axis=1).reshape(-1)
            self.torch_value = tensor(self.value)
            self.vec_flag = 0
        return self.value
    
    def parse_value(self, threshold=1e-5):
        if np.abs(self.value) < threshold:
            self.value = np.atleast_1d(0.0)
        self.name = "parse_term"
        return self.value
    ...
    def __repr__(self):
        if self.value is None:
            return self.name
        if self.vec_flag == 1:
            if np.var(self.value, axis=1) < 1e-4:
                self.value = np.mean(self.value, axis=1).reshape(-1)
                self.vec_flag = 0
