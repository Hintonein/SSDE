...
class Jacobian:
    ...
    def __call__(self, i=0, j=None):
        ...
        if not 0 <= i < self.dim_y:
            raise ValueError("i={} is not valid. dim_y is {}.".format(i, self.dim_y))
        ...
        if i not in self.J:
            ...
            self.J[i] = torch.autograd.grad(y,
                                            self.xs,
                                            grad_outputs=torch.ones_like(y),
                                            create_graph=True)[0]

        return (self.J[i] if j is None or self.dim_x == 1 else self.J[i][:,
                                                                         j:j +
                                                                         1])
...
def jacobian(ys, xs, i=0, j=None):
    temp = jacobian._Jacobians(ys, xs, i=i, j=j)
    clear()
    return temp
...
class Hessian:
    ...
    def __init__(self, y, xs, component=None, grad_y=None):
        ...
        if dim_y > 1:
            ...
            if component >= dim_y:
                raise ValueError(
                    "The component of y={} cannot be larger than the dimension={}."
                    .format(component, dim_y))
        else:
