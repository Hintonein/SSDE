...
import ssde
from ssde.prior import LengthConstraint
from ssde.program import Program
from ssde.utils import import_custom_source
from ssde.prior import JointPrior
from ssde.tf_state_manager import StateManager
...
def make_policy(sess, prior, state_manager, policy_type, **config_policy):
    ...
    if policy_type == "rnn":
        from ssde.policy.rnn_policy import RNNPolicy
        ...
        assert issubclass(policy_class, Policy), \
                "Custom policy {} must subclass ssde.policy.Policy.".format(policy_class)
...
class Policy(ABC):
    ...
    distribution over discrete objects. ssde algorithms optimize the parameters 
    ...
    def __init__(self, 
        ...
        state_manager: ssde.tf_state_manager.StateManager
