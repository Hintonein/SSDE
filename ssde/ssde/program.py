...
import numpy as np
from ssde.library import Token, PlaceholderConstant, Polynomial
from ssde.const import make_const_optimizer
from ssde.utils import cached_property
...
class Program(object):
    ...
    def _init(self, tokens, on_policy=True):
        ...
        self.bc_mse = None # Results of the constants pre-optimization
    ...
    def recursion_optimize(self, reoptim=False):
        """
        ...
        if self.task.ablation:
            reoptim = True

        def f(consts):
            consts = consts.reshape(-1, self.task.n_param)
            self.set_constants(consts)
            r = self.task.reward_function(self, optimizing = not reoptim)
            obj = -r
            self.invalid = False
            return obj
        ...
        if not reoptim:
            x0 = np.ones(len(self.const_pos) * self.task.n_param)
            optimized_constants, temp = Program.const_optimizer(f, x0)
            self.optimized_constants = optimized_constants
            # return the inital bc error as the initial evaluation
            self.bc_mse = temp
            if self.bc_mse == -1 or np.isnan(self.bc_mse):
                self.bc_mse = -1
                return 0
            return self.task.reward_function(self, dt=False)
        else:
            initial_guess = [tensor(np.ones(self.task.n_input_var), requires_grad=True) for _ in self.const_pos]
            optimized_constants, mse = make_const_optimizer('torch')(pde_f, initial_guess)
            self.optimized_constants = optimized_constants
            self.set_constants(self.optimized_constants)
            # if not self.task.ablation:
            #     # real reoptimization
            #     old_reward = self.r
            #     reward = (self.task.reward_function(self) + old_reward)/2
            # else:
            reward = self.task.reward_function(self)

            if reward > 0.999:
                print('Const Pruning Optimization ......')
                for i in self.const_pos:
                    self.traversal[i].const_prune()
                initial_guess = [tensor(self.traversal[i].value, requires_grad=True) for i in self.const_pos]
                optimized_constants, mse = make_const_optimizer('torch')(pde_f, initial_guess)
                self.set_constants(optimized_constants)
                new_reward = self.task.reward_function(self)
                # new_reward = (self.task.reward_function(self)+ old_reward)/2
                if new_reward < reward:
                    print('Finetuning failed, reward:', new_reward)
                    self.set_constants(self.optimized_constants)
    ...
    def optimize(self, reoptim=False):
        """
        ...
        # directly use pde error to optimize the constants
        if self.task.task_type=="solver" and self.task.ablation:
            reoptim = True
        ...
        if not reoptim:
            ...
            if self.optimized_constants is None:
                self.optimized_constants = np.ones(len(self.const_pos))
            x0 = [tensor(x0, requires_grad=True) for x0 in self.optimized_constants]
            optimized_constants, temp = make_const_optimizer('torch')(f, x0)
            reward = self.task.reward_function(self)
            object.__setattr__(self, 'r', reward)
    ...
    def set_execute(cls, protected):
        ...
        from ssde.execute import python_execute
        execute_function = python_execute

        try:
            from ssde.execute import cython_execute
        ...
        try:
            from ssde.execute import cython_recursion_execute
            execute_recursion_function = cython_recursion_execute
            Program.have_recursion_cython = True
        except ImportError:
            Program.have_recursion_cython = False
            ...
            def unsafe_execute(traversal, X):
                ...
                with np.errstate(all='log'):
                    if isinstance(X, Tensor):
                        sympde_execute_function = python_execute
                        y = sympde_execute_function(traversal, X)
                    elif Program.task.task_type == "recursion":
                        y = execute_recursion_function(traversal, X)
                    else:
    ...
    def r(self):
        ...
        with warnings.catch_warnings():
            ...
            if Program.task.task_type == "recursion":
                return self.recursion_optimize()
